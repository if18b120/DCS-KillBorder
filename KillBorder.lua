...
function KillBorder:generate(ReferenceGroup, Coalition, Side, GroupIdentifier, Time, WarnDistance, PunishType, PunishTimer, Sound)
	...
	self.flexiborder[1] = {x = V.x * 1000000 + self.flexiborder[2].x, z = V.z * 1000000 + self.flexiborder[2].z}

	V = getV2(self.flexiborder[count - 1], self.flexiborder[count])
	self.flexiborder[count] = {x = V.x * 1000000 + self.flexiborder[count - 1].x, z = V.z * 1000000 + self.flexiborder[count - 1].z}
